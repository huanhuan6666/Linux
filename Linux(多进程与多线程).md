...
但是这里很快结束我更倾向于是`sleep()`库函数被信号打断，**sleep也能被信号打断**，返回**剩余秒数**，打断之后就不睡了，因此远小于10s就结束。sleep中的pause将进程设置成**可中断睡眠态**，然后执行内核调度函数`schedule()`，硬件中断、信号都可以**唤醒**。
> 因为sleep库函数内部是由`pause()`进程阻塞直到捕获一个信号和`alarm()`一定时间后发送SIGALRM信号来实现的，我们手动发一个信号进程就不必等闹钟了。其实也可以看作是信号打断了阻塞的系统调用pause，我们的wait函数也是这样，也可能被除了SIGCHLD的信号**打断**，因此wait也要避免EINTR**假错**，需要额外处理

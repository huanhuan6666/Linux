...
>    当然这其实是个拜占庭将军问题，C端和S端想要就某个问题达成共识，这里就是**序列号**，好比将军问题中的共同出击敌人的共识一样，永远都要有一侧承担风险，比如C端最后回应ACK后仍然无法确定 这个包到底有没有被S端收到（也就是谁发最后一个包谁承担风险），但是由于之前C->S和S->C两次握手成功让C端可以姑且认为S端收到了自己最后的消息。
>    
